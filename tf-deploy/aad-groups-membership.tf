...
resource "azuread_group_member" "repository_function_app_member_of_portal_database_readers" {
  group_object_id  = azuread_group.sql_portal_database_readers_group.id
  member_object_id = azurerm_function_app.repository_function_app.identity[0].principal_id
}

resource "azuread_group_member" "repository_webapi_member_of_portal_database_readers" {
  group_object_id  = azuread_group.sql_portal_database_readers_group.id
